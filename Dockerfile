...
RUN echo "cp -a /mnt/{.git,build,Makefile} . && find . -type f -print0 | xargs -0 dos2unix -ic0 | xargs -0 dos2unix -b && source /root/emsdk/emsdk_env.sh && make && rm -rf /mnt/dist && mkdir /mnt/dist && cp ffmpeg*.js ffmpeg*.wasm /mnt/dist/" > /opt/build.sh
